...
def load_bikes(hours_in_advance=1):
    df = pd.read_csv("data/bicikelj/bicikelj_weathernan.csv")
    #holiday = (df["holiday"].values == None).astype(np.float32)
    ...
    #set target variable to be the number of bikes in the future instead of the current number of bikes
    if hours_in_advance > 1:
        hours_in_advance=hours_in_advance-1
        future_traffic_volume = np.roll(traffic_volume, -hours_in_advance)
        future_traffic_volume[-hours_in_advance:] = traffic_volume[-hours_in_advance]
        traffic_volume = future_traffic_volume
    ...
    features = np.stack([temp, rain, clouds, weekday, hour_of_day,day_of_week,prev_hour_bikes], axis=-1)
...
class TrafficData:
    def __init__(self, seq_len=32, exclude_timestamps=None, hours_in_advance=1):

        date_time, x, y = load_bikes(hours_in_advance=hours_in_advance)
...
# def predict_bikes(timestamp):
#     # Convert timestamp to datetime object
#     timestamp = dt.datetime.strptime(timestamp, "%Y-%m-%d %H:%M:%S")

#     # Load the dataset
#     date_time,features, traffic_volume = load_bikes()
...
def predict_bikes(timestamp,hours_in_advance):
    # Convert timestamp to datetime object
    timestamp = dt.datetime.strptime(timestamp, "%Y-%m-%d %H:%M:%S")

    # Load the dataset
    date_time,features, traffic_volume = load_bikes(hours_in_advance=hours_in_advance)
...
def predict_bikes2(timestamp,hours_in_advance):
    # Convert timestamp to datetime object
    timestamp = dt.datetime.strptime(timestamp, "%Y-%m-%d %H:%M:%S")
    ...
    # Pass the features through the model to make a prediction
    prediction = model2.sess.run(model2.y, {model2.x: closest_features})
...
if __name__ == "__main__":
    ...
    traffic_data = TrafficData(exclude_timestamps=timestamps,hours_in_advance=1)
    ...
    for timestamp in timestamps:
        ...
        if i % 2 == 0:
            pred=predict_bikes(timestamp,hours_in_advance=1)
    ...
    traffic_data2 = TrafficData(exclude_timestamps=timestamps,hours_in_advance=2)
    model2 = TrafficModel(model_type=args.model, model_size=args.size)
    model2.fit(traffic_data, epochs=args.epochs, log_period=args.log)
    model2.restore()
    ...
    for timestamp in timestamps:
        ...
        if i % 2 == 0:
            ...
            pred=predict_bikes2(timestamp,hours_in_advance=2)
            preds.append(pred)
